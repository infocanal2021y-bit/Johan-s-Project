...
const AdminBankTransfersPage = () => {
    ...
    const [selected, setSelected] = useState(null); // currently viewing proof
    const [proofBlob, setProofBlob] = useState(null); // {b64, mime, filename}
    const [rejecting, setRejecting] = useState(null); // currently rejecting (proof obj)
    ...
    const fetchList = async () => {
        setLoading(true);
        try {
            const { data } = await api.get('/admin/bank-transfer-proofs', { params: filter ? { status: filter } : {} });
